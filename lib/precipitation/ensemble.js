...
import { round2, lnGamma } from '../core/utils.js';
 ...
 * Uses the standard continued fraction representation (Numerical Recipes §6.2).
 * Q(a,x) = exp(-x + a*ln(x) - lnΓ(a)) * (1/(x+1-a+ 1*(1-a)/(x+3-a+ 2*(2-a)/(x+5-a+ ...))))
 */
function gammaCFQ(a, x) {
  const lnGammaA = lnGamma(a);
  const TINY = 1e-30;
  
  // Modified Lentz's method for the CF:
  // b0 = x + 1 - a
