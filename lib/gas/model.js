...
import { round2, normalCDF } from '../core/utils.js';

// ── Seasonal factors ─────────────────────────────────────────────────────────
// Default seasonal adjustment (cents/gallon above/below annual mean)
// Source: EIA 5-year average seasonal pattern — used as prior, can be overridden by data
const DEFAULT_SEASONAL_CENTS = {
  1: -15, 2: -12, 3: -5, 4: 5, 5: 15, 6: 20,
  7: 18, 8: 12, 9: 5, 10: -5, 11: -10, 12: -15,
};

// Will be overwritten by estimateSeasonality() when enough data is available
let SEASONAL_CENTS = { ...DEFAULT_SEASONAL_CENTS };
...
/**
 * Estimate seasonal pattern from historical gas price data.
 * Computes average price per month, then deviation from overall mean.
 * Blends 50/50 with default prior to avoid overfitting on short history.
 */
function estimateSeasonality(gasHistory) {
  const byMonth = {};
  for (const g of gasHistory) {
    const m = parseInt(g.date.slice(5, 7));
    if (!byMonth[m]) byMonth[m] = [];
    byMonth[m].push(g.price);
  }
  const overall = gasHistory.reduce((s, g) => s + g.price, 0) / gasHistory.length;
  for (let m = 1; m <= 12; m++) {
    if (byMonth[m] && byMonth[m].length >= 2) {
      const monthMean = byMonth[m].reduce((a, b) => a + b, 0) / byMonth[m].length;
      const dataCents = Math.round((monthMean - overall) * 100);
      // Blend 50% data + 50% prior
      SEASONAL_CENTS[m] = Math.round((dataCents + DEFAULT_SEASONAL_CENTS[m]) / 2);
    }
  }
}
...
export function calibrate(gasHistory, crudeHistory) {
  ...
  // Estimate seasonality from data if we have ≥52 weeks
  if (gasHistory.length >= 52) {
    estimateSeasonality(gasHistory);
  }
 ...
 */
/**
 * EIA → AAA bias correction.
 *
 * Kalshi gas contracts (KXAAAGASW/M) settle on the **AAA national average**,
 * but our data source is the **EIA weekly retail price**. EIA prices are
 * systematically higher than AAA by ~$0.08–0.12/gallon (different survey
 * methodology, station sampling, and timing).
 *
 * Without this correction the model is biased upward: it predicts gas "above"
 * thresholds that AAA actually settles below. This caused a 0/12 loss streak
 * in Feb 2026 paper trading.
 *
 * The offset is an approximation — ideally we'd switch to AAA data directly,
 * but AAA has no public API. Until then, subtract this from our EIA-based
 * prediction before comparing to Kalshi thresholds.
 */
const EIA_AAA_OFFSET = 0.15; // dollars/gallon (EIA > AAA)
// Empirically, the gap was $0.15-0.20 in Feb 2026 paper trading
// (all 12 gas trades lost with $0.10 offset — AAA settled well below model predictions)
...
export function predict(params) {
  ...
  // 5. Combine adjustments + apply EIA→AAA bias correction
  const predictedChange = meanRevAdj + trendAdj + crudeAdj;
  const predictedMean = currentGas + predictedChange - EIA_AAA_OFFSET;

  // 6. Uncertainty: sigma scales with sqrt(weeks)
  // Add extra uncertainty for EIA→AAA offset variance (~$0.04 std)
  // The offset itself is uncertain by ±$0.04, plus model error
  const rawSigma = cal.weeklySigma * Math.sqrt(weeksAhead);
  const dataSourceUncertainty = 0.06; // AAA vs EIA measurement noise (±6¢)
  const modelUncertainty = 0.08; // fair-value model calibration error (±8¢)
  const baseSigma = Math.max(
    0.12, // floor: never be more confident than ±12¢
    // Total uncertainty = sqrt(price_vol² + data_source² + model_error²)
    Math.sqrt(rawSigma * rawSigma + dataSourceUncertainty * dataSourceUncertainty + modelUncertainty * modelUncertainty)
  );
...
export function probAbove(predictedMean, sigma, threshold, cal) {
  ...
  // Use symmetric sigma — the old asymmetric adjustment was wrong.
  // cal.upSigma/downSigma measure dispersion *within* directional moves,
  // not the overall probability of up/down moves. Using them as scaling
  // factors made the model dramatically overconfident (e.g. halving downside
  // sigma when downside moves were actually consistent but large).
  const z = (predictedMean - threshold) / sigma;

  // Clamp to avoid extreme confidence (max ~98%)
  const clampedZ = Math.max(-2.5, Math.min(2.5, z));
  return normalCDF(clampedZ);
}
...
// normalCDF imported from core/utils.js
