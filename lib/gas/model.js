...
export function predict(params) {
  ...
  const dataSourceUncertainty = 0.04; // AAA vs EIA measurement noise
  const baseSigma = Math.max(
    0.06, // floor: never be more confident than ±6¢ (prevents pEst ≈ 1.0)
    Math.sqrt(rawSigma * rawSigma + dataSourceUncertainty * dataSourceUncertainty)
  );
