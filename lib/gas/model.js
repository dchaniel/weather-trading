 ...
 */
const EIA_AAA_OFFSET = 0.10; // dollars/gallon (EIA > AAA)
...
export function predict(params) {
  ...
  // Add extra uncertainty for EIA→AAA offset variance (~$0.02 std)
  const rawSigma = cal.weeklySigma * Math.sqrt(weeksAhead);
  const baseSigma = Math.sqrt(rawSigma * rawSigma + 0.02 * 0.02);
