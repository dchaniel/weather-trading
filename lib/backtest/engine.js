...
export function generatePredictions(data, opts = {}) {
  ...
  for (const err of data.errors) {
    ...
    const rawMean = fc.ecmwf != null && fc.gfs != null
      ? fc.ecmwf * 0.6 + fc.gfs * 0.4
      : fc.mean;
    // Apply station bias correction (same as live forecast code in forecast.js)
    // s.bias is the correction value (negated raw bias): if forecasts are 8°F cold,
    // s.bias = +8. We ADD it to correct the forecast upward, matching live code:
    //   adjustedMean = weightedMean + (s.bias || 0)
    const forecastMean = rawMean + (s?.bias || 0);
    ...
    for (let offset = -thresholdRange; offset <= thresholdRange; offset += thresholdStep) {
      ...
      // P(YES) = P(temp < threshold) to match Kalshi convention (YES = below)
      const predicted = normalCDF((threshold - forecastMean) / sigma);
...
export function simulatePnL(predictions, opts = {}) {
  ...
  for (const pred of predictions) {
    ...
    // Market prob = P(YES) = P(temp < threshold) to match Kalshi convention
    const marketProb = normalCDF((pred.threshold - pred.forecastMean) / marketSigma);
