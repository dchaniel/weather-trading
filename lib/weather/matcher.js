...
/**
 * Blend model sigma with market-implied sigma for more calibrated probabilities.
 * When the market disagrees with our sigma, trust the market partially.
 * Weight: 40% model, 60% market (market has been more accurate historically).
 */
function blendSigma(modelSigma, marketSigma) {
  if (!marketSigma || marketSigma <= 0) return modelSigma;
  // Floor: never go below model sigma (avoid overconfidence from thin markets)
  const blended = 0.4 * modelSigma + 0.6 * marketSigma;
  return Math.max(blended, modelSigma);
}
...
export function scoreContract(mkt, fc, station, date, sigma, marketSigma, balance, customMinEdge) {
  ...
  if (parsed.type === 'threshold') {
    pTrue = probAboveThreshold(c.adjustedMean, parsed.threshold, effectiveSigma);
