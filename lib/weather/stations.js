 ...
 * - Mar 12: Re-calibrated from 30-day MAE (Feb 10 - Mar 12). Mar 9 values
 *   were 2-3x too high for KMIA, KIAH, KSFO, KDEN, KSEA. New formula:
 *   baseSigma = MAE × 1.3 - winterBump, floor 1.5°F. KNYC & KATL unchanged
 *   (already well-calibrated at 1.07x and 0.92x respectively).
 * - Mar 14: Re-calibrated using RESIDUAL MAE (post-bias-correction) from ASOS
 *   validation data. Previous values used raw MAE which double-counted the
 *   systematic bias that forecast.js already corrects. Formula:
 *   baseSigma = estimated_residual_MAE × 1.5, floor 2.0°F.
 *   Residual estimated as sqrt(MAE² - bias²) with floor at MAE*0.3.
 *   Key fixes: KSFO 1.5→3.4, KSEA 1.5→2.9, KMIA 1.5→2.0, KATL 2.5→2.0.
 *
 ...
 * IMPORTANT: Calibration based on N=7 per station — expand ASOS validation
 * sample size as more data accumulates for more reliable estimates.
 *
