 ...
 *
 * ── Calibration note ──
 * baseSigma values were re-calibrated Mar 3 2026 to account for the gap between
 * Open-Meteo gridded forecasts and ASOS point observations (Kalshi settlement source).
 * Previous values (all 2.0°F) were derived from Open-Meteo forecast vs Open-Meteo
 * archive, which dramatically underestimated true forecast error because the forecast
 * and "actuals" came from the same model family. Real ASOS errors are 4-6°F depending
 * on station. Coastal/microclimate stations (KSFO, KMIA, KLAX) get higher sigma due
 * to grid-vs-point marine influence. Mountain/convective stations (KDEN, KOKC) get
 * higher sigma for weather variability. These are conservative estimates — better to
 * miss some edge than to be overconfident and lose every trade (0/25 loss streak
 * in Feb 2026 paper trading was caused by this miscalibration).
 *
