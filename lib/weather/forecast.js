...
export function buildConsensus(forecasts, station, month, horizonDays = 0, tempType = 'high') {
  ...
  const s = STATIONS[station];
  // s.bias is the correction value (negated raw bias) from calibrate-asos-bias.js
  // If OM forecast is cold by 8°F, s.bias = +8, so we ADD it to correct upward
  //
  // Confidence-weighted bias: shrink correction toward 0 when calibration N is small.
  // With N=7 (current), we only apply ~50% of the raw bias to avoid overcorrecting
  // from a lucky/unlucky sample. Full correction requires N>=30.
  const rawBias = s.bias || 0;
  const biasN = s.biasN || 7;  // calibration sample size
  const MAX_BIAS_F = 10.0;     // absolute cap — allow large corrections for stations like KNYC (8°F bias confirmed over 60 days)
  const shrinkage = Math.min(1.0, biasN / 30);  // linear ramp: full trust at N>=30
  const cappedBias = Math.sign(rawBias) * Math.min(Math.abs(rawBias), MAX_BIAS_F);
  const effectiveBias = round1(cappedBias * shrinkage);
  const adjustedMean = round1(weightedMean + effectiveBias);
  ...
  if (Math.abs(climDev) > CLIM_OUTLIER_THRESHOLD_F) {
    result.tradeable = false;
    result.reason = `Forecast ${adjustedMean.toFixed(1)}°F is ${Math.abs(climDev).toFixed(1)}°F from normal (${normalTemp}°F)`;
