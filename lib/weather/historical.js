...
export function simulateForecast(actualHigh, station, month) {
  const s = STATIONS[station];
  // s.bias is the bias CORRECTION (e.g. +8 means "add 8°F to raw forecast").
  // Raw forecast ≈ actual - bias + noise (simulates the model's systematic error).
  // Adjusted forecast = raw + bias = actual + noise (bias cancels out).
  // Previous bug: returned raw forecast as adjustedMean, missing the +bias step.
  const bias = s.bias || 0;
  const noise = (Math.sin(actualHigh * 17.3) * 0.5 + Math.sin(actualHigh * 7.1) * 0.3) * 1.5;
  const adjustedForecast = actualHigh + noise; // bias cancels: (actual - bias + noise) + bias
  const spread = Math.abs(noise) * 0.8;
  return {
    adjustedMean: round1(adjustedForecast),
