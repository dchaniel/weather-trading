...
const KEY_PATH = process.env.KALSHI_KEY_PATH || '/home/daniel/.openclaw/workspace/skills/kalshi/kalshi_key.pem';
...
/**
 * Get market resolution status and result.
 * Returns { resolved, result, status, expirationValue, settlementValue } or null on error.
 */
export async function getMarketResolution(ticker) {
  try {
    const m = await getMarket(ticker);
    const resolved = ['settled', 'finalized'].includes(m.status);
    return {
      resolved,
      result: m.result || null,        // "yes" or "no"
      status: m.status,
      expirationValue: m.expiration_value ? parseFloat(m.expiration_value) : null,
      settlementValue: m.settlement_value,
