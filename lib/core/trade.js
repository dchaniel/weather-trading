...
export function executeTrade(station, contract, side, qty, price, strategy = 'weather', metadata = {}) {
  ...
  // Deduplication: block if we already have an unsettled position on this contract+side
  const existingPosition = ledger.trades.find(t =>
    !t.settled && !t.duplicate && t.contract === contract && t.side === side
  );
  if (existingPosition) {
    throw new Error(`Duplicate trade blocked: already have unsettled ${side} position on ${contract} (trade #${existingPosition.id})`);
  }
...
export function settleDate(date, actuals) {
  ...
  for (const trade of ledger.trades) {
    if (trade.settled || trade.duplicate) continue;
    ...
    if (isThreshold) {
      ...
      // Bracket: use stored floorStrike/capStrike from API if available,
      // otherwise fall back to ticker parsing via parseTicker()
      let lo, hi;
      if (trade.floorStrike != null && trade.capStrike != null) {
        lo = trade.floorStrike;
        hi = trade.capStrike;
      } else {
        // Fallback: parse from ticker (parseTicker handles the math)
        const parsed = parseTicker(trade.contract);
        lo = parsed?.bracketLow ?? Math.floor(bracket);
        hi = parsed?.bracketHigh ?? Math.ceil(bracket) + (bracket === Math.floor(bracket) ? 1 : 0);
      }
...
/** Get total P&L from settled trades (excludes duplicates) */
export function getTotalPnL() {
  const ledger = loadLedger();
  return ledger.trades
    .filter(t => t.settled && t.pnl != null && !t.duplicate)
