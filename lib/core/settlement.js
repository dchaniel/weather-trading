...
/**
 * Determine contract settlement outcome.
 *
 * Kalshi threshold ("-T") markets use strike_type "less":
 *   YES wins when actual temp < threshold (below)
 *   NO  wins when actual temp >= threshold (at or above)
 *
 * Bracket ("-B") markets: YES wins when actual is in/above the bracket.
 *
 * @param {number} actualTemp - observed temperature
 * @param {number} threshold - contract strike value
 * @param {string} [contractType='bracket'] - 'threshold' or 'bracket'
 */
export function determineOutcome(actualTemp, threshold, contractType = 'bracket') {
  let outcome;
  if (contractType === 'threshold') {
    // Kalshi -T markets: YES = below threshold
    outcome = actualTemp < threshold ? 'YES' : 'NO';
  } else {
    // Bracket markets: YES = at or above
    outcome = actualTemp >= threshold ? 'YES' : 'NO';
  }
  return { outcome, actualHigh: actualTemp, threshold };
}
...
async function checkKalshiResolution(ticker) {
  try {
    ...
    if (['settled', 'finalized'].includes(market.status) && market.result) {
      return {
        resolved: true,
        result: market.result.toLowerCase(),
        expirationValue: market.expiration_value ? parseFloat(market.expiration_value) : null,
...
export async function executeSettlement(date) {
  ...
  if (weatherFallbackTrades.length > 0) {
    ...
    for (const trade of weatherFallbackTrades) {
      ...
      const threshold = parsed?.threshold ?? parsed?.bracket ?? (tMatch ? parseFloat(tMatch[1]) : bMatch ? parseFloat(bMatch[1]) : null);
      if (threshold == null) continue;

      const contractType = (parsed?.type === 'threshold' || tMatch) ? 'threshold' : 'bracket';
      const { outcome, actualHigh } = determineOutcome(actual.high_f, threshold, contractType);
...
export async function verifyAllSettledTrades() {
  ...
  for (const trade of settled) {
    ...
    results.push({
      ...
      kalshiExpirationValue: resolution.expirationValue,
      kalshiStrikeType: resolution.strikeType,
      kalshiTitle: resolution.title,
      kalshiWon,
      kalshiPnl,
      ourPnl: trade.pnl,
