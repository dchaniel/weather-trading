...
const MIN_SIGMA_RATIO = gcfg.minSigmaRatio ?? 1.5;
...
export function runGuards({ station, qty, forecastSpread, marketSigma, forecastHigh, date, bidAskSpread }) {
  ...
  if (station && STATIONS[station]) {
    ...
    if (marketSigma != null) {
      ...
      if (ratio < MIN_SIGMA_RATIO) {
        reasons.push(`Market σ ratio ${ratio.toFixed(2)}x (${marketSigma.toFixed(1)}°F / ${ourSigma.toFixed(1)}°F) < required ${MIN_SIGMA_RATIO}x — insufficient edge`);
      }
    } else {
  ...
  const warnings = [];
  const month = parseInt(date.slice(5, 7));
  const CALIBRATION_MONTHS = new Set([11, 12, 1, 2, 3]); // Must match WINTER_MONTHS in stations.js
  if (!CALIBRATION_MONTHS.has(month)) {
    warnings.push(`⚠️ CALIBRATION WARNING: Trading outside calibration window (Nov-Mar). baseSigma values were calibrated on winter data only — summer forecast errors may be higher, especially for convective stations (KOKC, KDFW, KIAH, KDEN).`);
  }
