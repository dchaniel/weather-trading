...
export async function executeApprovedTrades(combined, blockedWeather, dates, sessionId) {
  ...
  for (const trade of executable) {
    try {
      ...
      try { appendTrade({ date: dates[0], station: trade.station, contract: trade.ticker, side: trade.side.toLowerCase(), qty: cappedContracts, price: trade.price, expectedEdge: trade.edge, marketSigma: trade.marketSigma, ourSigma: trade.sigma, forecastTemp: trade.forecastTemp ?? trade.forecastHigh, pEst: trade.pEst, threshold: trade.threshold }); } catch {}
